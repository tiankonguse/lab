...
//构造微云分享地址
preg_match('|url=\/(\w+)\.|', $_SERVER['REQUEST_URI'], $res);
...
preg_match('|"thumb_url":"([^"]+)"|', $src, $res);
$url = $res ? $res[1] : exit("Unable to get source code!");
...
function curl_get_contents($url) {
    ...
    curl_setopt($ch, CURLOPT_REFERER, "http://share.weiyun.com/");
